...
?>
/**
 * <?php echo '@TO' . 'DO'; ?>: Import <?php echo "$name" ?> description from `Docs/model_descriptions.md`.
 */
...
if (!empty($validate)):
	...
	foreach ($validate as $field => $validations):
		...
		foreach ($validations as $key => $validator):
			...
			echo "\t\t\t\t'message' => '@TO" . "DO: Validation message for `$name.$field` ($key) not defined.',\n"; // Strategically break the line in this template so we don't list this in phpdoc as a pending task.
			if ($firstRule) {
				$firstRule = false;
